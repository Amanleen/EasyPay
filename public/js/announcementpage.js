/* Client side code for postAnnouncement */
/* Author: sv + aman, October 2015 */
...
$(function(){
    ...
    function onUpdateUserRole(userrole){
    	if(userrole=='admin' || userrole=='cord'){
            $('#postAnnouncementAnnounce-btn').removeClass('hidden');
            $('#postAnnouncementMsg-ta').removeClass('hidden');
        }
    }
	...
	$('#postAnnouncementAnnounce-btn').on('click',function(){
		var content=$('#postAnnouncementMsg-ta').val();
		if(content!==null && content.length >0)
		{
			...
			console.log(msg);
			con.request("/anno", msg, function(data, code) {
			    console.log(code);
			    if(code==201){
			...
			$(document).ready(function(){
			    $('#postAnnouncementMsg-ta').val('');
			});
		}
	});

	//SEARCH BUTTON ON CLICK
	$('#postAnnouncementSearch-btn').on('click', function(){
		var searchQuery = $('#postAnnouncementSearch-ip').val().trim();
        ...
        if(!isBanned){
        	con.request("/anno/search",toSearch,function(data, code){
        		console.log(data.length);
        		$('#announcementHistory-ul li').remove();
        		if(data.length>0){
	        		appendToUI(data);
	        	}else{
	        		$('#announcementHistory-ul').append('<li class="list-group-item">No Result found</li>');
	        	}
        	});
        }
	});
	...
	//PpostAnnouncement  history
   con.request("/anno/wall",{},function(data, code){
	...
	con.subscribe("anno.new", function(data){
            ...
            $(document).ready(function(){
                $('#announcementHistory-ul').append('<li class='+statusColorClass+'><span class="chat-img pull-left"></span><div class="chat-body clearfix"><div class="header"><strong class="primary-font">'+from+'</strong> <small class="pull-right text-muted"><span class="glyphicon glyphicon-time"></span>&nbsp;'+creationTime+'</small></div><p>'+content+'</p><hr class="hr-clas-low"></div></li>');
            });
        });
});