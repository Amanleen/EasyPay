        ...
        $('#loginForm-btn').click(function() {
		  ...
		  if ($('#password1-ip').val().trim().length < '4') {
           ...
           if (!isInvalid) {
			...
			con.request('/user/login', {userName: $('#username-ip').val().trim(), password: $('#password1-ip').val().trim()}, function(data, code) {
					if (code == 401) {
					  $("#errorBlock2-sp").text("Password entered is wrong. Try again");
			          $("#errorBlock2-sp").show();
					}
					else if (code == 404) {
					  $("#errorBlock3-sp").text("User does not exist. Try again or go back to signup");
			          $("#errorBlock3-sp").show();
					}
					else if (code == 200) {
						var username = $('#username-ip').val().trim();
